...
# 1. Enable memory growth (most important!)
...
def load_ascad(dpa_database_file, load_metadata=False, validation_split=5000, load_attack=True):
    ...
    try:
        ...
        print("Error: can't open HDF5 file '%s' for reading" % dpa_database_file)
        sys.exit(-1)

    # Load profiling (training) set
    X_profiling = np.array(in_file['Profiling_traces/traces'], dtype=np.float32)
    Y_profiling = np.array(in_file['Profiling_traces/labels'])

    # If attack data is not needed (training mode)
    if not load_attack:
        if validation_split > 0:
            # Load only the first validation_split traces as the validation set
            print(f"\n Dataset split (training mode - validation set only):")
            print(f"  Profiling set: {X_profiling.shape[0]} traces")
            print(f"  Loading validation set: first {validation_split} attack traces...")

            X_validation = np.array(in_file['Attack_traces/traces'][:validation_split], dtype=np.float32)
            Y_validation = np.array(in_file['Attack_traces/labels'][:validation_split])

            print(f"  Validation set: {X_validation.shape[0]} traces")
            print(f"  Remaining attack data not loaded (saves ~21 GB of memory)")
            ...
            # No validation set needed; return profiling set only
            print(f"\n Dataset split (training mode - no validation set):")
            print(f"  Profiling set: {X_profiling.shape[0]} traces")
            print(f"  Attack data not loaded")
    ...
    # Load full attack data (evaluation mode)
    print(f"\n Dataset split (evaluation mode - full data loaded):")
    ...
    if validation_split > 0:
        # First validation_split traces → validation set
        X_validation = X_attack_full[:validation_split]
        Y_validation = Y_attack_full[:validation_split]

        # Remaining traces → attack test set
        X_attack = X_attack_full[validation_split:]
        Y_attack = Y_attack_full[validation_split:]

        print(f"  Profiling set:  {X_profiling.shape[0]} traces")
        print(f"  Validation set: {X_validation.shape[0]} traces (first {validation_split} attack traces)")
        print(f"  Test set:       {X_attack.shape[0]} traces (remaining {len(Y_attack_full) - validation_split} attack traces)")
        ...
        # No validation split; preserve original behaviour
        if load_metadata:
...
def Conv_1D_Block_APS(x, model_width, kernel, strides=1, padding="same"):
    """Convolutional block with APS downsampling"""
    x = tf.keras.layers.Conv1D(
...
output_nums = 256  # Number of AES S-box output classes


# ==================== Main network: standard receptive field version (RF=79) ====================
def Inceptionv3_APS_StandardReceptive_RF79(num_filters=64):
    """Standard receptive field version: designed for Desync0 (no random delay), RF=79"""
    input_shape = (20000, 1)
    ...
    # Classification head with light dropout
    ...
    print(f"   Output: {output_nums} classes (softmax)")
    # Build model
    model = tf.keras.Model(inputs, outputs, name='InceptionNet_APS')
...
# ==================== Training function with validation set support ====================
def train_model(X_profiling, Y_profiling, model, save_file_name,
    ...
    Args:
        X_profiling: training features
        Y_profiling: training labels
        model: Keras model
        save_file_name: path to save the model
        X_validation: validation features (optional)
        Y_validation: validation labels (optional)
    ...
    if X_validation is not None:
        # With validation set: save the best model based on val_accuracy
        ...
        print(f"     (Monitor: val_accuracy, strategy: save best only)")

        # Save a full checkpoint every 5 epochs
        ...
        callbacks.append(
            tf.keras.callbacks.ModelCheckpoint(
                checkpoint_path,
                monitor='val_accuracy',
                save_best_only=False,  # Save every 5 epochs, not just the best
                save_freq='epoch',
                period=5,
                verbose=1
            )
        )
        print(f"   ModelCheckpoint: save model every 5 epochs to checkpoints/")
...
def find_latest_checkpoint(model_folder, model_name_base):
    """Find the most recent checkpoint file for a given model."""
    # Build a search pattern specific to this model (not all checkpoints)
    ...
    for ckpt in checkpoints:
        try:
            epoch_str = ckpt.rsplit("_epoch", 1)[1].split(".h5")[0]  # Split from the right
...
if __name__ == "__main__":
    # File paths
    ASCAD_data_folder = "./DPA/DPA_dataset/"
    ASCAD_trained_models_folder = "./DPA/DPA_trained_models/"
    history_folder = "./DPA/training_history/"
    ...
    # ==================== Load data (without loading the full attack set) ====================
    print("Loading DPA Desync0 dataset (TRAINING MODE)...")
    (X_profiling, Y_profiling), (X_validation, Y_validation), _ = load_ascad(
        ASCAD_data_folder + "DPA_20k_desync0.h5",
        load_metadata=False,
    ...
    print(f"\n Final dataset sizes:")
    print(f"  Profiling set: {X_profiling.shape}")
    print(f"  Validation set: {X_validation.shape}")
    print(f"  Number of classes: {len(np.unique(Y_profiling))}")

    # Check memory usage
    print(f"\n Memory usage:")
    print(f"  X_profiling:  {X_profiling.nbytes / 1024 ** 3:.2f} GB")
    print(f"  Y_profiling:  {Y_profiling.nbytes / 1024 ** 3:.2f} GB")
    print(f"  X_validation: {X_validation.nbytes / 1024 ** 3:.2f} GB")
    print(f"  Y_validation: {Y_validation.nbytes / 1024 ** 3:.2f} GB")

    total_data_gb = (X_profiling.nbytes + Y_profiling.nbytes +
                     X_validation.nbytes + Y_validation.nbytes) / 1024 ** 3
    print(f"  Total data size: {total_data_gb:.2f} GB")
    ...
    model_name = f"InceptionNet_APS_DPA_desync0_20k_epochs{epochs}_batchsize{batch_size}.h5"
    ...
    initial_epoch = 0  # Default: start from epoch 0

    if latest_checkpoint is not None and completed_epoch < epochs:
        # Checkpoint found and training not yet complete — resume training
        print("\n" + "=" * 80)
        print("RESUMING FROM CHECKPOINT")
        print("=" * 80)
        print(f"Found checkpoint at epoch {completed_epoch}")
        print(f"File: {os.path.basename(latest_checkpoint)}")
        print(f"Resuming training from epoch {completed_epoch} to {epochs}")
        ...
        print("=" * 80 + "\n")
        print("Options:")
        print("   1. Use the existing model for evaluation")
        print("   2. Increase 'epochs' to continue training")
        print("   3. Delete checkpoints to retrain from scratch")
        sys.exit(0)
    ...
    # Verify learning rate is correctly configured
    print(f"\n Verifying optimizer configuration...")
    actual_lr = model.optimizer.learning_rate.numpy()
    print(f"   Actual learning rate: {actual_lr:.6f}")
    if abs(actual_lr - 0.0001) < 1e-7:
        print(f"    Learning rate correctly set to 0.0001")
    else:
        print(f"   ️Warning: Expected 0.0001, got {actual_lr:.6f}")
    ...
    # ==================== Use a generator to avoid memory overflow ====================
    ...
    def data_generator(X, Y, batch_size, shuffle=True):
        """On-demand data generator — processes one batch at a time."""
        indices = np.arange(len(X))

        while True:  # Infinite loop so Keras can iterate over multiple epochs
            if shuffle:
            ...
            for start_idx in range(0, len(X), batch_size):
                ...
                # Load only the current batch (memory: batch_size × 20000 × 8 bytes ≈ 8 MB)
                batch_x = X[batch_indices].reshape(-1, 20000, 1).astype(np.float32)
    ...
    # Print model summary only when starting from scratch
    if initial_epoch == 0:
        print("\n" + "=" * 80)
        print("MODEL SUMMARY")
        print("=" * 80)
    ...
    print(f"Validation:     Enabled ({X_validation.shape[0]} samples)")
    print(f"Training data:  {X_profiling.shape[0]} samples ({X_profiling.nbytes / 1024 ** 3:.2f} GB)")
    print("=" * 80 + "\n")
    ...
    history = model.fit(
        ...
        epochs=epochs,
        initial_epoch=initial_epoch,  # Key: enables resuming from checkpoint
        verbose=1,
        callbacks=callbacks,
    ...
    # Save 1: Final model (last epoch)
    final_save_path = ASCAD_trained_models_folder + model_name.replace('.h5', '_final.h5')
    print(f"\n1. Saving FINAL model (last epoch): {os.path.basename(final_save_path)}")
    ...
    try:
        ...
        if os.path.exists(final_save_path):
            ...
            print("   ️Warning: FINAL model file not found after save!")
    ...
    # Save 2: Best model (highest val_accuracy) — already saved automatically by ModelCheckpoint
    best_save_path = ASCAD_trained_models_folder + model_name.replace('.h5', '_best.h5')
    print(f"\n2. BEST model (highest val_accuracy): {os.path.basename(best_save_path)}")
    ...
    if os.path.exists(best_save_path):
        ...
        print(f"   Warning: BEST model not found at {best_save_path}")
        print(f"   This may happen if training did not complete any epochs")
    ...
    # Display final training results (including validation metrics)
    ...
    if 'val_accuracy' in history.history:
        ...
        if acc_gap > 0.05:
            print(f"\n   ⚠️  Overfitting detected: train-val accuracy gap = {acc_gap * 100:.2f}%")
        else:
            print(f"\n   ✅ Good generalization: train-val accuracy gap = {acc_gap * 100:.2f}%")
